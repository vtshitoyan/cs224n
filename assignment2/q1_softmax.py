...
def softmax(x):
    ...
    ### YOUR CODE HERE
    exponents = tf.exp(x - tf.reduce_max(x, axis=1, keepdims=True))  # subtracting max of each vector
    out = exponents/tf.reduce_sum(exponents, axis=1, keepdims=True)  # summing over vector dimensions
    # important not to do any operations along the n_samples dimension
    ### END YOUR CODE
